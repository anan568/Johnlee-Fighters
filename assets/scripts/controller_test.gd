...
func _input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("light_0"):
		print_debug("hey")
	if Whitelisted_Pressed():
...
func Whitelisted_Pressed():
	for action in whitelisted_moves:
		if Input.is_action_just_pressed(action + "_%s" % [device_id]):
			action_just_pressed = action
			#$RichTextLabel.add_text(action + "\n")
			return true
	return false
